...
const config = require('../config/services').gambitConversations;

/**
 * @param {object} apiRes - Gambit Conversations v1 api response
 * @return {object}
 */
function formatV1IndexResponse(apiRes) {
  const total = Number(apiRes.header[config.resultsCountHeader]) || 0;
  return {
    data: apiRes.body,
    meta: {
      pagination: {
